package com.example.basic;

import java.util.Random;

import android.app.Activity;
import android.graphics.Color;
import android.os.Bundle;
import android.text.InputType;
import android.view.Gravity;
import android.view.View;
import android.widget.Button;
import android.widget.EditText;
import android.widget.TextView;
import android.widget.ToggleButton;

//public class TextPlay extends Activity {
// Example of implementing methods 
public class TextPlay extends Activity implements View.OnClickListener {

	Button chkCmd;
	ToggleButton passTog;
	EditText input;
	TextView display;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		// TODO Auto-generated method stub
		super.onCreate(savedInstanceState);
		setContentView(R.layout.text);

		// Example of how to create methods
		baconAndEggs();

		
		passTog.setOnClickListener(this);
		chkCmd.setOnClickListener(this);
		//The above method can be done as per the commented code below, if View.OnClickListene() was not implemented
		// Setup OnClickListener and auto-generate onClick() from View
		// chkCmd.setOnClickListener(new View.OnClickListener() {
		//
		// @Override
		// public void onClick(View v) {
		// TODO Auto-generated method stub
		// String check = input.getText().toString();
		// //Set the Text
		// display.setText(check);
		// if(check.contentEquals("left")) {
		// //Gravity Class
		// display.setGravity(Gravity.LEFT);
		// }else if (check.contentEquals("center")) {
		// display.setGravity(Gravity.CENTER);
		// }else if (check.contentEquals("right")) {
		// display.setGravity(Gravity.RIGHT);
		// }else if (check.contentEquals("blue")) {
		// //Changing Text Color
		// display.setTextColor(Color.BLUE);
		// }else if (check.contentEquals("WTF")){
		// //Random Class
		// Random crazy = new Random();
		// display.setText("WTF!!!!");
		// //Randomness between 75 values
		// display.setTextSize(crazy.nextInt(75));
		// display.setTextColor(Color.rgb(crazy.nextInt(255),
		// crazy.nextInt(255), crazy.nextInt(255)));
		// switch(crazy.nextInt(3)) {
		// case 0:
		// display.setGravity(Gravity.LEFT);
		// break;
		// case 1:
		// display.setGravity(Gravity.CENTER);
		// break;
		// case 2:
		// display.setGravity(Gravity.RIGHT);
		// break;
		// }
		// } else {
		// display.setText("invalid");
		// display.setGravity(Gravity.CENTER);
		// }
		// }
		// });

	}

	// Example of creating a method
	private void baconAndEggs() {
		// TODO Auto-generated method stub
		chkCmd = (Button) findViewById(R.id.bResults);
		passTog = (ToggleButton) findViewById(R.id.tbPassword);
		input = (EditText) findViewById(R.id.etCommand);
		display = (TextView) findViewById(R.id.tvResult);

	}

	@Override
	public void onClick(View view) {
		// TODO Auto-generated method stub
		switch (view.getId()) {
		case R.id.tbPassword:
			if (passTog.isChecked()) {
				// Password
				input.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
			} else {
				// Regular Text
				input.setInputType(InputType.TYPE_CLASS_TEXT);
			}
			break;

		case R.id.bResults:
			String check = input.getText().toString();
			// Set the Text
			display.setText(check);
			if (check.contentEquals("left")) {
				// Gravity Class
				display.setGravity(Gravity.LEFT);
			} else if (check.contentEquals("center")) {
				display.setGravity(Gravity.CENTER);
			} else if (check.contentEquals("right")) {
				display.setGravity(Gravity.RIGHT);
			} else if (check.contentEquals("blue")) {
				// Changing Text Color
				display.setTextColor(Color.BLUE);
			} else if (check.contentEquals("WTF")) {
				// Random Class
				Random crazy = new Random();
				display.setText("WTF!!!!");
				// Randomness between 75 values
				display.setTextSize(crazy.nextInt(75));
				display.setTextColor(Color.rgb(crazy.nextInt(255),
						crazy.nextInt(255), crazy.nextInt(255)));
				switch (crazy.nextInt(3)) {
				case 0:
					display.setGravity(Gravity.LEFT);
					break;
				case 1:
					display.setGravity(Gravity.CENTER);
					break;
				case 2:
					display.setGravity(Gravity.RIGHT);
					break;
				}
			} else {
				display.setText("Invalid Input");
				display.setGravity(Gravity.CENTER);
				display.setTextColor(Color.BLACK);
			}
			break;
		}
	}
}
